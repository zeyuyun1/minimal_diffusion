...
def find_target_neuron(a_final, constrain_img_xy=None, constrain_win=6):
    """Find the most activated fitted neuron.

    constrain_img_xy: optional (img_x, img_y) — restrict search to feature
        cells whose receptive-field centre is within `constrain_win` feature
        pixels of this image location.  Useful when the globally strongest
        neuron is a corner artifact and we want to focus on a specific region.
    """
    best_val = -1.0
    ...
    for lvl, a_lvl in enumerate(a_final):
        ...
        IMG_H = a_lvl.shape[2] * (2 ** (lvl + 1))   # approx image height
        IMG_W = a_lvl.shape[3] * (2 ** (lvl + 1))   # approx image width
        for ch in range(a_lvl.shape[1]):
            if (lvl, ch) not in fitted_set:
                continue
            spatial = a_lvl[0, ch].abs()
            if constrain_img_xy is not None:
                cx_img, cy_img = constrain_img_xy
                # Convert image pixel → feature coords
                cx_feat = cx_img / IMG_W * W_lvl
                cy_feat = cy_img / IMG_H * H_lvl
                # Peak within the spatial window
                y0 = max(0,     int(cy_feat - constrain_win))
                y1 = min(H_lvl, int(cy_feat + constrain_win) + 1)
                x0 = max(0,     int(cx_feat - constrain_win))
                x1 = min(W_lvl, int(cx_feat + constrain_win) + 1)
                region = spatial[y0:y1, x0:x1]
                if region.numel() == 0:
                    continue
                val = region.max().item()
                if val > best_val:
                    best_val = val
                    ry = region.argmax() // region.shape[1]
                    rx = region.argmax() %  region.shape[1]
...
for stem in STIMULI:
    ...
    # ── Target neuron ─────────────────────────────────────────────────────────
    # Per-stimulus spatial constraints: restrict search to the region of interest
    # rather than picking the global maximum (which can land on corner artifacts).
    CONSTRAIN = {
        "shape_Circle_perturbed": (28, 46),    # midpoint of two blurs at ~200°/213°
    }
    constrain_xy = CONSTRAIN.get(stem, None)
    target_level, target_channel, target_y, target_x = find_target_neuron(
        a_final, constrain_img_xy=constrain_xy, constrain_win=6)
    print(f"  Target: L{target_level} ch{target_channel} @ (y={target_y}, x={target_x})"
          + (f"  [constrained to px~{constrain_xy}]" if constrain_xy else ""))
    ...
    # ── Header thumbnails: clean / noisy / denoised ──────────────────────────
    def _img_arr(t):
        """Tensor [1,H,W] or [H,W] → normalised uint8 HxWx3."""
        a = t.squeeze().cpu().float().numpy()
    ...
    # ── 6th header panel: zoomed clean image (no needle) ─────────────────────
